...
def get_role():
    ...
    PendingOrder = db.session.query(Orders).join(OrderStatus).filter(OrderStatus.OrderStatus == 'Pending').count()
    NewOrders = db.session.query(Orders).join(OrderStatus).filter(OrderStatus.OrderStatus == 'Pending').all()

    return render_template('role.html', RoleItems = RoleItems, PendingOrder = PendingOrder, NewOrders = NewOrders)
